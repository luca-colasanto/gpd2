...
void Cloud::voxelizeCloud(double cell_size) {
  ...
  voxels.row(0) = voxels.row(0) * cell_size +
                  Eigen::VectorXf::Ones(voxels.cols()) * min_xyz(0);
  voxels.row(1) = voxels.row(1) * cell_size +
                  Eigen::VectorXf::Ones(voxels.cols()) * min_xyz(1);
  voxels.row(2) = voxels.row(2) * cell_size +
                  Eigen::VectorXf::Ones(voxels.cols()) * min_xyz(2);
  ...
  if (normals_.cols() > 0) normals_ = normals;
}
...
void Cloud::calculateNormals(int num_threads) {
  ...
  // reverse direction of normals (if a normal does not point to at least one
  // camera)
  std::cout << "Reversing direction of normals that do not point to at least "
               "one camera ...\n";
  reverseNormals();
}
